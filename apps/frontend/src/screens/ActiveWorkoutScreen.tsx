...
function SetRow({ index, weight, reps, state }: SetRowProps) {
  return (
    <View
      className={`flex-row items-center gap-2 rounded-sm mt-1 p-2.5 px-3 ${
        state === 'active' ? 'bg-surface-hover border border-accent' : ''
      }`}
    >
      <Text
        className={`w-8 mono-sm ${
      ...
      <View className="flex-row items-center justify-between py-2.5 px-4 bg-surface-raised border-b border-border">
        ...
        <View className="flex-row gap-1">
          <Pressable
            ...
            className="px-2.5 h-7 items-center justify-center rounded-sm"
          >
