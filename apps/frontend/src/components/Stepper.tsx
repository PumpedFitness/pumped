    ...
    <View className="gap-1.5">
      {(label || unit) && (
        <View className="flex-row justify-between items-baseline">
          ...
          {unit && !isSmall && (
            <Text className="eyebrow opacity-70">{unit}</Text>
          )}
        </View>
      )}
      <View
        className={`flex-row overflow-hidden rounded-sm bg-field-background border ${
          pressed ? 'border-accent' : 'border-border'
        } ${isSmall ? 'h-[52px]' : 'h-16'}`}
      >
        <Pressable
          onPressIn={() => startRamp(-1)}
          onPressOut={endRamp}
          disabled={disabled || value <= min}
          accessibilityLabel={`Decrease ${label || 'value'}`}
          className={`items-center justify-center ${
            isSmall ? 'w-12' : 'w-[52px]'
          } ${pressed === 'dec' ? 'bg-accent-soft' : ''} ${
        ...
        >
          <SvgIcon
            d={Icons.minus}
            size={iconSize}
            color={pressed === 'dec' ? colors.accent : colors.textSecondary}
            strokeWidth={2.5}
          />
        </Pressable>
        ...
        <View className="flex-1 items-center justify-center border-x border-border px-1.5">
          <Text
            className={
              isSmall
                ? 'font-mono text-xl font-semibold text-foreground tracking-tight'
                : 'mono-display'
            }
          >
            {formatValue(value)}
        ...
        >
          <SvgIcon
            d={Icons.plus}
            size={iconSize}
            color={pressed === 'inc' ? colors.accent : colors.textSecondary}
            strokeWidth={2.5}
          />
        </Pressable>
      </View>
